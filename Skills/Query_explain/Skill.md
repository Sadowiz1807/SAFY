...
name: query_explain
version: 1.0.0
description: "Explains SQL drafts without executing them."
enabled: true
risk_level: low
references: []
---
...
## Purpose
Explains SQL drafts without executing them.

## When to use
Use this skill when SAFY routes a user request to `query_explain` in the normal Perceive → Plan → Slot-fill → Route → Act → Verify → Present → Remember workflow.
...
## Expected output
Return the normal SAFY response envelope or action result for `query_explain`.
