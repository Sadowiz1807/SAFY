...
def make_check(sql: str, target: str, database_profile_id: str | None, access_mode: str):
    ...
    sql_hash = "hash_runtime_" + hashlib.sha256(sql.encode("utf-8")).hexdigest()[:16]
    check_id = "check_runtime_" + uuid.uuid4().hex[:16]
    ...
    warnings = []
    if high:
        warnings.append("Destructive or mutating SQL requires test-support manual confirmation.")
    elif warning:
        warnings.append("Statement is allowed in test-support mode but should be reviewed.")
    else:
        warnings.append("Read-only style query detected by test-support checker.")
    if blocked:
        warnings.append("Selected test-support access mode is disabled.")
    ...
    return {
        ...
        "safety_report": {
            ...
            "permission_expectation": "Test-support execution follows selected user query access mode.",
            "warnings": warnings,
        },
    }
